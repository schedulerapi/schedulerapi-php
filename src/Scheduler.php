...
class Scheduler
{
    ...
    private function update($data) {
        ...
        $ch = curl_init('https://api.schedulerapi.com/update');
        curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
        curl_setopt($ch, CURLINFO_HEADER_OUT, true);
        curl_setopt($ch, CURLOPT_POST, true);
        curl_setopt($ch, CURLOPT_POSTFIELDS, $payload);
     ...
     * @throws Exception
     */
    public function updateWebhook($id, $when, $url, $method, $body, $config = [])
    {
        if (count($config) > 0) {
            $this->applyConfig($config);
        }
     ...
     * @throws Exception
     */
    public function updateSqs($id, $when, $url, $body, $config = [])
    {
        if (count($config) > 0) {
            $this->applyConfig($config);
        }
